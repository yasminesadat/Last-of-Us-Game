	...
	public static void endTurn() throws NotEnoughActionsException, InvalidTargetException {
		...
		for (int i = 0; i < map.length; i++)
			for (int j = 0; j < map[i].length; j++){
				map[i][j].setVisible(false);  //change for testing
			}
		...
		for(Cells c:Cells.curedCells){
			c.setCharacterHandler();
		}
		Cells.curedCells.clear();
	}
...
//		engine.Game.map[1][1]=new CharacterCell(z);
		adjustVisibility(h);
	}
